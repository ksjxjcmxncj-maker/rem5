...
//                    }
//                    int effId = player.zone.effDragon;
...
//    }
    ...
    public void sendInfo(Session session) {
        ...
        Boss bubbles = BossManager.gI().getBossById(Util.createIdDuongTank((int) player.id) + 250000);
        if (bubbles == null) {
            try {
                Boss_Bubbles dt = new Boss_Bubbles(Util.createIdDuongTank((int) player.id), BossData.BUBBLES, player.zone, -1, -1, player.id);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            ...
            if (player.gender == 0) {
                npcSay += "Mình là Obito sẽ đồng hành cũng bạn trên thế giới này\n";
...
//        }
        if (player.isPl()) {
            if (player.inventory.itemsBody.get(13).isNotNullItem()) {
                new Thread(() -> {
                    try {
                        Thread.sleep(1000);
                        Service.getInstance().sendFoot(player, (short) player.inventory.itemsBody.get(13).template.id);
                    } catch (Exception e) {
                    }
                }).start();
            }
        ...
        SendEffect.getInstance().sendThreadDanhHieu(player);
