...
public class EffectSkin {
    ...
    public void update() {
        updateVoHinh();
        if (this.player.effectSkin != null && this.player != null && this.player.zone != null && !this.player.zone.map.isMapOffline && this.player.zone.map.mapId != 46) {
            updateOdo();
        ...
        if (isMaPhongBa && (Util.canDoWithTime(lastTimeMaPhongBa, timeMaPhongBa))) {
            EffSkinService.gI().removeMaPhongBa(this.player);
        }
