...
public class NgoKhong extends Boss {
    ...
    public void rewards(Player pl) {
        ...
        if (Util.isTrue(10, 100)) {
            ...
            RewardService.gI().initBaseOptionClothes(itemMap.itemTemplate.id, itemMap.itemTemplate.type, itemMap.options);
        } else if (Util.isTrue(11, 10)) {
            int[] set3= {547,548};
            itemMap = new ItemMap(this.zone, set3[Util.nextInt(0, set3.length - 1)], 1, x, y, pl.id);

            itemMap.options.add(new ItemOption(77, Util.nextInt(2, 20)));
            itemMap.options.add(new ItemOption(103, Util.nextInt(20, 20)));
            itemMap.options.add(new ItemOption(50, Util.nextInt(25, 20)));
