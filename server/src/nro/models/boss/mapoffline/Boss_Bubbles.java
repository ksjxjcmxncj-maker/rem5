 ...
 * @copyright 💖 GirlkuN 💖
 */
public class Boss_Bubbles extends FutureBoss {

    public Boss_Bubbles(int bossID, BossData bossData, Zone zone, int x, int y, long id_player) throws Exception {
        super(bossID + 250000, bossData);
        this.zone = zone;
        this.location.x = x;
        this.location.y = y;
    ...
    public void update() {
        try {
            ...
            if (pl2 == null) {
                ChangeMapService.gI().changeMap(this, 48, 0, 375, 235);
            }
            if (!this.effectSkill.isHaveEffectSkill() && !this.effectSkill.isCharging) {
                switch (this.status) {
                    ...
                    case JUST_JOIN_MAP:
                        ...
                        PlayerService.gI().playerMove(this, 375, 235);
                        if (this.zone != null) {
    ...
    public void joinMap() {
        if (this.zone != null) {
            ChangeMapService.gI().changeMap(this, 45, 0, 375, 235);
            canJoinMap = true;
            changeStatus(REST);
        }
    }
