...
//    }
    public void sendImageByName(Session session, String imgName) {
        ...
        try {
            ...
            byte[] data = FileIO.readFile("resources/data_game/img_by_name/x" + session.zoomLevel + "/" + imgName + ".png");
            msg.writer().writeInt(data.length);
            msg.writer().write(data);
            session.sendMessage(msg);
            msg.cleanup();
    ...
    public static void effData(Session session, int id, int... idtemp) {
        ...
        try {
            byte[] effData;
            byte[] effImg;
            effData = FileIO.readFile("resources/data_game/Eff/effect/x" + session.zoomLevel + "/data/DataEffect_" + idT);
            effImg = FileIO.readFile("resources/data_game/Eff/effect/x" + session.zoomLevel + "/img/ImgEffect_" + idT + ".png");
            if (id == 25) {
