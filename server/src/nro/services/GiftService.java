...
public class GiftService {
    ...
    public static synchronized GiftService gI() {
        if (i == null) {
            i = new GiftService();
        }
        return i;
        ...
        synchronized (code.intern()) {
            try (Connection _gcon = DBService.gI().getConnectionForGame();
                 PreparedStatement stmt = _gcon.prepareStatement("SELECT * FROM `gift_codes` WHERE `code` like ? AND (expires_at IS NULL OR expires_at > now()) LIMIT 1;",
                ...
                try (ResultSet res = stmt.executeQuery()) {
                    ...
                    sb.append("|7|Bạn nhận được").append("");
                    for (int i = 0; i < size; i++) {
                        ...
                        sb.append(String.format("|2|%s %s", Util.numberToMoney(quantity), item.template.name)).append("");
                    }

                    if (gold > 0) {
                        player.inventory.addGold(gold);
                        sb.append(String.format("- %s vàng", Util.numberToMoney(gold))).append("");
                    }

                    if (gem > 0) {
                        player.inventory.addGem(gem); // FIX
                        sb.append(String.format("- %s ngọc xanh", Util.numberToMoney(gem))).append("");
                    }

                    if (ruby > 0) {
                        player.inventory.addRuby(ruby); // FIX
                        sb.append(String.format("- %s hồng ngọc", Util.numberToMoney(ruby))).append("");
                    }
...
");
        ...
        try (Connection _gcon = DBService.gI().getConnectionForGame();
             PreparedStatement stmt = _gcon.prepareStatement("SELECT * FROM `gift_code_histories` WHERE `gift_code_id` = ? AND `player_id` = ? LIMIT 1;", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
        ...
        try (Connection _gcon = DBService.gI().getConnectionForGame();
             PreparedStatement stmt = _gcon.prepareStatement("INSERT INTO `gift_code_histories`(`player_id`, `gift_code_id`, `code`, `created_at`) VALUES (?, ?, ?, ?)")) {
            Timestamp timestamp = new Timestamp(System.currentTimeMillis());
