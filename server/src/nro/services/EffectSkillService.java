...
public class EffectSkillService {
    ...
    public void downBienHinh(Player player) {
        ...
        Skill sBienHinh = player.playerSkill.getSkillbyId(27);
        int skillLevel = sBienHinh.point;
        boolean lastLevel = player.effectSkill.levelBienHinh >= skillLevel - 1;
        if (!lastLevel) {
            Skill template = Manager.NCLASS.get(player.gender).getSkillTemplate(sBienHinh.template.id).skillss.stream().filter(s -> s.point == sBienHinh.point).findFirst().orElse(null);
...
//        player.effectSkill.timeAnTroi = timeAnTroi;
    ...
    public void setIsMonkey(Player player) {
        ...
        if (player.setClothes.cadic == 5) {
            timeMonkey += timeMonkey * player.setClothes.tilecadic;
        }
